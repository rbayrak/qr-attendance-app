...
const AttendanceSystem = () => {
  ...
  const handleStudentIdChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newId) {
      ...
      setStatus(isValid ? '✅ Öğrenci numarası doğrulandı' : '⚠️ Bu öğrenci numarası listede yok');
    }
  }, [validStudents]);
  ...
  const handleQrScan = useCallback(async (decodedText: string) => {
    try {
      ...
      if (!validStudents.some(s => s.studentId === studentId)) {
  ...
  }, [studentId, location, validStudents, updateAttendance]);
          ...
          <div className="bg-white p-6 rounded-xl shadow-md space-y-4">
            ...
            <div className="space-y-4">
              ...
              {isScanning && (
                <div 
                  ref={qrReaderRef}
                  className="relative w-full aspect-square bg-gray-200 rounded-xl overflow-hidden"
                >
